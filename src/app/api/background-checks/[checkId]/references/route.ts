...
export async function POST(
  ...
  const { type, name, manager_name, phone, email, address, contact_date, status, notes, income_monthly } = body
  if (!type) return Response.json({ error: 'type required' }, { status: 400 })

  const { data, error } = await supabaseAdmin
    .from('bg_check_references')
    .insert({ bg_check_id: checkId, type, name, manager_name: manager_name || null, phone, email, address, contact_date: contact_date || null, status: status || 'pending', notes, income_monthly: income_monthly || null })
    .select()
    .single()
...
export async function PATCH(
  ...
  const allowed = ['name','manager_name','phone','email','address','contact_date','status','notes','income_monthly']
