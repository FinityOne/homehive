...
export async function POST(req: NextRequest, { params }: { params: Promise<{ leadId: string }> }) {
  ...
  const { data: prop } = await supabaseAdmin
    .from('properties')
    .select('id, name, address, price, owner_id, rental_mode, property_images(url, position)')
...
<div style="max-width:560px;margin:0 auto;padding:32px 16px;">
  ...
  return Response.json({ success: true, accept_token: reservation.accept_token, acceptUrl })
}
