...
export default function LandlordLeadsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'insights' | 'leads' | 'roommates'>('overview')
  ...
  // ─── Roommate Groups ─────────────────────────────────────────────────────────
  type RoommateGroup = { id: string; name: string; description: string | null; property_slug: string | null; emoji: string; created_at: string; member_count: number }
  type GroupMember = {
    id: string; lead_id: string; notes: string | null; added_at: string; has_prescreen: boolean
    leads: { id: string; first_name: string | null; last_name: string | null; email: string; phone: string | null; status: Lead['status']; property: string | null; created_at: string | null; move_in_date: string | null } | null
  }
  ...
  useEffect(() => {
    if (activeTab === 'roommates' && userId) loadRoommateGroups()
  }, [activeTab, userId, loadRoommateGroups])
      ...
      <div className="ll-page">
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #e8e5de', padding: '0 28px', display: 'flex', gap: 0 }}>
          {([
            ...
            { id: 'roommates', label: `Roommate Groups${roommateGroups.length > 0 ? ` (${roommateGroups.length})` : ''}` },
          ] as const).map(tab => (
            <button
              key={tab.id}
            ...
            {selectedGroup ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#1a1a1a', letterSpacing: '-0.3px' }}>Roommate Groups</div>
                  ...
                  <button
                    ...
                    style={{ background: '#8C1D40', color: '#fff', border: 'none', borderRadius: 9, padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}
                  >
                    + New Group
                  </button>
                </div>
                ...
                {roommateLoading ? (
                  ...
                  <div style={{ background: '#fff', border: '1.5px dashed #e8e5de', borderRadius: 16, padding: '60px 40px', textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
                    ...
                    <button
                      onClick={() => setCreateGroupModal(true)}
                      style={{ background: '#8C1D40', color: '#fff', border: 'none', borderRadius: 9, padding: '12px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
                    >
                      Create First Group
                    </button>
                  </div>
